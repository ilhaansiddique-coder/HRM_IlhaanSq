...
export async function permanentDeleteProduct(tenantId: string, id: string) {
  const existing = await prisma.product.findFirst({
    where: { id, tenantId, isDeleted: true },
  });
  if (!existing) throw new Error("Not found in trash");
  await prisma.product.delete({ where: { id } });
}

export async function permanentDeleteSale(tenantId: string, id: string) {
...
export async function permanentDeleteCustomer(tenantId: string, id: string) {
...
export async function listCourierProviders(tenantId: string) {
...
export async function upsertCourierProvider(
  tenantId: string,
  input: {
    provider: string;
    isEnabled: boolean;
  ...
  return prisma.courierProvider.upsert({
    ...
    create: {
      ...
      refreshInterval: input.refreshInterval ?? "hourly",
    },
  });
}
