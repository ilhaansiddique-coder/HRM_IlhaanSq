...
type CustomerHistorySalePayment = {
  method: string | null;
  amount: number | null;
};
...
type CustomerHistorySale = {
  ...
  created_at: string;
  sale_payments?: CustomerHistorySalePayment[] | null;
};

const normalizeSalesHistoryRows = (rows: unknown[], includePaymentTerms: boolean): CustomerHistorySale[] => {
  return rows.map((row) => {
    const sale = row as Record<string, any>;
    ...
    return {
      ...
      review_amount_paid: sale.review_amount_paid ?? null,
      review_amount_due: sale.review_amount_due ?? null,
      payment_method: sale.payment_method ?? null,
      payment_terms: includePaymentTerms ? sale.payment_terms ?? null : null,
...
export const CustomerHistoryDialog = ({
  ...
  const [sales, setSales] = useState<CustomerHistorySale[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const primarySalesResult = await supabase
        .from("sales")
      ...
      let fetchError = primarySalesResult.error;
      let salesData = normalizeSalesHistoryRows(primarySalesResult.data || [], true);
      ...
      if (fetchError) {
        ...
        if (missingCreditTerms) {
          const fallbackSalesResult = await supabase
            .from("sales")
          ...
          fetchError = fallbackSalesResult.error;
          salesData = normalizeSalesHistoryRows(fallbackSalesResult.data || [], false);
        }
      }
