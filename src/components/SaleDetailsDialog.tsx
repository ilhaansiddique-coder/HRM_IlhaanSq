...
export const SaleDetailsDialog = ({ open, onOpenChange, saleId, previewOverride }: SaleDetailsDialogProps) => {
  ...
  const creditSplitTotal = normalizedSplits
    ...
    .reduce((sum, split) => sum + split.amount, 0);
  const effectiveAmountPaid = Number(previewOverride?.amount_paid ?? sale.amount_paid) || 0;
  const effectiveAmountDue = Number(previewOverride?.amount_due ?? sale.amount_due) || 0;
  const creditDisplayAmount =
    creditSplitTotal > 0
      ? creditSplitTotal
      : sale.payment_terms === "credit"
        ? Math.max(0, effectiveAmountDue)
        : 0;
  const isCreditSale =
  ...
  const displayPaidAmount =
    paidSplitTotal > 0
      ...
      : isCreditSale
        ? 0
        : Math.max(0, effectiveAmountPaid);
  const displayDueAmount = Math.max(0, effectiveAmountDue);
