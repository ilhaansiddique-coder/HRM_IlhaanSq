...
import { useProducts, Product, CreateProductData } from "@/modules/inventory/hooks/useProducts";
import { useBusinessSettings } from "@/hooks/useBusinessSettings";
import { Loader2, X, Plus, Package, DollarSign, Layers, History } from "lucide-react";
import { ImagePicker } from "@/components/ImagePicker";
import { Switch } from "@/components/ui/switch";
import { Badge } from "@/components/ui/badge";
import { useProductVariants, AttributeDefinition, type ProductVariant } from "@/modules/inventory/hooks/useProductVariants";
...
export const ProductDialog = ({ open, onOpenChange, product }: ProductDialogProps) => {
  ...
  const ensureUniqueSku = async (candidate: string, excludeId?: string | null) => {
    ...
    for (let i = 0; i < 50; i += 1) {
      let query = supabase.from("products").select("id").eq("sku", next).limit(1).maybeSingle();
      if (excludeId) {
                          ...
                          <div key={idx} className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="space-y-2">
                                  <div className="flex gap-2">
                                  <Input
                                    ref={(el) => attributeInputRefs.current[idx] = el}
