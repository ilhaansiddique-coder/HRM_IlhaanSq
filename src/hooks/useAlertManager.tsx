...
export const useAlertManager = () => {
  ...
  const loadDismissedAlerts = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`dismissed-alerts-${user.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const saveDismissedAlert = useCallback(async (alertId: string) => {
    try {
      ...
      if (user) {
        const { data: existingDismissal, error: existingDismissalError } = await supabase
          .from('dismissed_alerts')
          .select('id')
          .eq('user_id', user.id)
          .eq('alert_id', alertId)
