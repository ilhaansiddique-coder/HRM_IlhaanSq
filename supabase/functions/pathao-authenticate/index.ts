...
serve(async (req)=>{
  ...
  try {
    ...
    const rateLimit = checkRateLimit(
      ...
      { ...RateLimitPresets.sensitive, keyPrefix: 'pathao-authenticate' }
    );
    ...
    const { data: existingSettings } = await supabaseClient.from('courier_webhook_settings').select('id').eq('tenant_id', authContext.tenantId).limit(1);
    const updatePayload = {
    ...
    if (existingSettings?.[0]?.id) {
      const { error: updateError } = await supabaseClient.from('courier_webhook_settings').update(updatePayload).eq('id', existingSettings[0].id).eq('tenant_id', authContext.tenantId);
      if (updateError) {
