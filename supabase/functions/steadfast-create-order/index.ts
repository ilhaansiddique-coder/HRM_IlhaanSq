...
serve(async (req)=>{
  ...
  try {
    ...
    let authContext;
    try {
      authContext = await resolveTenantAuthContext(supabaseClient, accessToken);
    } catch (error) {
      return new Response(JSON.stringify({
        success: false,
    ...
    const rateLimit = checkRateLimit(
      ...
      { ...RateLimitPresets.sensitive, keyPrefix: 'steadfast-create-order' }
    );
    ...
    const { data: webhookSettings, error: settingsError } = await supabaseClient.from('courier_webhook_settings').select('steadfast_api_key, steadfast_secret_key, steadfast_enabled').eq('tenant_id', authContext.tenantId).maybeSingle();
    if (settingsError) {
    ...
    if (orderData.sale_id) {
      ...
      const { data: updateData, error: updateError } = await supabaseClient.from('sales').update(updatePayload).eq('id', orderData.sale_id).eq('tenant_id', authContext.tenantId).select('id, consignment_id, cn_number, tracking_number, courier_status').single();
      if (updateError) {
