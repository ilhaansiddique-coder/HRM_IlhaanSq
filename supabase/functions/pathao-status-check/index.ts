...
serve(async (req)=>{
  ...
  try {
    ...
    let authContext;
    try {
      authContext = await resolveTenantAuthContext(supabase, userAccessToken);
    } catch (error) {
      return new Response(JSON.stringify({
        success: false,
    ...
    const rateLimit = checkRateLimit(
      ...
      { ...RateLimitPresets.standard, keyPrefix: 'pathao-status-check' }
    );
    if (!rateLimit.allowed) {
      return rateLimitExceededResponse(rateLimit, corsHeaders);
    }
    const { data: sale, error: saleError } = await supabase.from('sales').select('id').eq('consignment_id', consignment_id).eq('tenant_id', authContext.tenantId).limit(1).maybeSingle();
    if (saleError || !sale) {
      return new Response(JSON.stringify({
        success: false,
    ...
    const { data: settings, error: settingsError } = await supabase.from('courier_webhook_settings').select('*').eq('tenant_id', authContext.tenantId).limit(1).single();
