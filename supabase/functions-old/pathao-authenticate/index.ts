...
serve(async (req) => {
    ...
    try {
        ...
        if (roleError || !userRole || !['admin'].includes(userRole.role)) {
            return new Response(
                JSON.stringify({ success: false, message: 'Only admins can configure Pathao integration' }),
                { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }

        const requestData: AuthenticateRequest = await req.json()
        ...
        if (!requestData.client_id || !requestData.client_secret) {
            return new Response(
                JSON.stringify({ success: false, message: 'client_id and client_secret are required' }),
        ...
        try {
            ...
            return new Response(
                JSON.stringify({ success: false, message: 'Invalid response from Pathao API', raw: text }),
        ...
        if (existingSettings?.[0]?.id) {
            ...
            if (updateError) {
                ...
                return new Response(
                    JSON.stringify({ success: false, message: 'Failed to save token: ' + updateError.message }),
                    { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
                )
            }
        } else {
            ...
            if (insertError) {
                ...
                return new Response(
                    JSON.stringify({ success: false, message: 'Failed to save token: ' + insertError.message }),
                    { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
                )
            }
        }
