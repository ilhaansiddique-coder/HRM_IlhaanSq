...
serve(async (req) => {
    ...
    try {
        ...
        if (roleError || !userRole || !['admin', 'manager', 'staff'].includes(userRole.role)) {
            return new Response(
        ...
        const { data: webhookSettings, error: settingsError } = await supabaseClient
            ...
            .select('id, pathao_access_token, pathao_refresh_token, pathao_token_expires_at, pathao_store_id, pathao_enabled, pathao_client_id, pathao_client_secret')
            .limit(1)
            .maybeSingle()
                ...
                if (webhookSettings.pathao_refresh_token && webhookSettings.pathao_client_id && webhookSettings.pathao_client_secret) {
                    ...
                    if (refreshResponse.ok) {
                        ...
                        await supabaseClient
                            ...
                            .eq('id', (webhookSettings as any).id)
        ...
        return new Response(
            JSON.stringify({
                ...
                consignment_id: consignmentId,
                tracking_code: trackingCode,
                status: 'not_sent', // Always return 'not_sent' for newly created orders
